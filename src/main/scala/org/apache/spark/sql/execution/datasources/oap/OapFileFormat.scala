    ...
    meta match {
      case Some(m) =>
        ...
        val enableVectorizedReader: Boolean =
          ...
          (m.dataReaderClassName.equals(OapFileFormat.ORC_DATA_FILE_CLASSNAME) &&
          sparkSession.sessionState.conf.getConf(OapConf.ORC_VECTORIZED_READER_ENABLED))) &&
        ...
        (file: PartitionedFile) => {
          ...
          var context: Option[VectorizedContext] = None
          if (enableVectorizedReader) {
            if (isParquet) {
              ...
              if (!requestedColIdsOrEmptyFile.isEmpty) {
                ...
                context = Some(OrcVectorizedContext(partitionSchema, file.partitionValues,
